...
export function WorkspaceSelector() {
  ...
  const handleSwitch = (ws: typeof workspaces[number]) => {
    if (ws.id === currentWorkspaceId) { setOpen(false); return }
    switchWorkspace(ws)
    setOpen(false)
  }
  ...
  return (
    <div className="relative w-full">
      <button ref={triggerRef} className="flex items-center gap-[10px] py-[9px] px-[10px] w-full text-left bg-[var(--surface)] border border-[var(--border-faint)] rounded-[10px] cursor-pointer transition-colors duration-120 hover:bg-[var(--surface-2)] hover:border-[var(--border-soft)]" onClick={handleOpen} type="button">
      ...
      {open && dropPos && createPortal(
        <>
          ...
          <div className="fixed z-[9999] w-[260px] bg-[var(--bg-2)] border border-[var(--border)] rounded-[13px] p-[6px] shadow-[0_24px_56px_-20px_oklch(0_0_0/0.7)] animate-in fade-in slide-in-from-top-1" style={{ position: 'fixed', top: dropPos.top, left: dropPos.left, zIndex: 9999 }}>
            ...
            <div className="flex flex-col gap-[1px] py-[4px] px-0">
              ...
              <button className="flex items-center gap-[8px] py-[7px] px-[10px] rounded-[8px] w-full text-left text-[12.5px] font-medium text-[var(--text-faint)] bg-transparent border-none cursor-pointer transition-colors duration-80 hover:bg-[var(--surface)] hover:text-[var(--text)] disabled:opacity-50 disabled:cursor-default" onClick={handleCreate} disabled={createWorkspace.isPending}>
            ...
            <button
              ...
              onClick={() => { setOpen(false); navigate(APP_ROUTES.WORKSPACE_SETTINGS) }}
            >
