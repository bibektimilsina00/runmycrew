...
from apps.api.app.repositories.workspace_repository import WorkspaceRepository
from apps.api.app.schemas.workspace import WorkspaceCreate, WorkspaceInviteCreate
from apps.api.app.services.email_service import EmailService, InviteEmail
...
class WorkspaceService:
    ...
    async def create_personal_workspace(self, user: User) -> Workspace:
        ...
        workspace = await self.repo.create_workspace(workspace_name, slug, user.id, is_personal=True)
        await self._ensure_default_workflow(workspace)
        return workspace
    ...
    async def require_edit(self, workspace_id: uuid.UUID, user: User) -> WorkspaceMember:
        ...
        if member.role not in EDIT_ROLES:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Workspace is read-only")
        return member
    ...
    def _assert_can_manage_target(
        ...
        if target.role == "owner" and target.user_id != actor.user_id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Owners cannot be modified")
        ...
        if new_role == "owner" and actor.role != "owner":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only owners can assign owner")
