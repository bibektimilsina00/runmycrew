"""OpenAI action node — OpenAI — chat, embeddings, images, audio.
...
MANIFEST = ProviderManifest(
    ...
    description="OpenAI — chat, embeddings, images, audio.",
    icon_slug="openai",
    color="#1c1c1c",
    ...
    operations=[
        OpSpec(
            ...
            visible_fields=["model", "messages", "temperature", "max_tokens"],
            body_builder=lambda v: {
                "model": getattr(v, "model", None) or "gpt-4o-mini",
                "messages": getattr(v, "messages", []) or [],
                "temperature": float(getattr(v, "temperature", 1) or 1),
                "max_tokens": (int(getattr(v, "max_tokens", 0) or 0) or None),
            },
        ),
        OpSpec(
            id="responses",
            label="Responses (o-series)",
            method="POST",
            path="/responses",
            visible_fields=["model", "input", "instructions"],
            body_builder=lambda v: {
                "model": getattr(v, "model", None) or "o4-mini",
                "input": getattr(v, "input", "") or "",
                "instructions": getattr(v, "instructions", None) or None,
            },
        ),
        OpSpec(
            id="embedding",
            ...
            method="POST",
            path="/embeddings",
            visible_fields=["model", "input"],
            body_builder=lambda v: {
                "model": getattr(v, "model", None) or "text-embedding-3-small",
                "input": getattr(v, "input", "") or "",
            },
        ),
        OpSpec(
            ...
            visible_fields=["prompt", "size", "quality", "n"],
            body_builder=lambda v: {
        ...
        OpSpec(
            ...
            visible_fields=["file_url", "model"],
            body_builder=lambda v: {
                "model": getattr(v, "model", None) or "whisper-1",
                "file": getattr(v, "file_url", "") or "",
            },
        ),
        OpSpec(
            id="moderation",
            ...
            method="POST",
            path="/moderations",
            visible_fields=["input"],
            body_builder=lambda v: {"input": getattr(v, "input", "") or ""},
        ),
        OpSpec(
            id="list_models",
            label="List Models",
