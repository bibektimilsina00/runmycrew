"""Guardrails AI action node — Guardrails AI — validate + repair LLM output against JSON schema.

REST at https://api.guardrailsai.com/v0. See sim-parity roadmap Phase 4.32.
"""
...
MANIFEST = ProviderManifest(
    ...
    color="#1c1c1c",
    base_url="https://api.guardrailsai.com/v0",
    credential_type="guardrails_api_key",
    token_field=["api_key"],
    auth="bearer",
    fields=[
    ...
    operations=[
        OpSpec(
            ...
            method="POST",
            path="/guards/{guard_name}/validate",
            visible_fields=["guard_name", "llm_output"],
            body_builder=lambda v: {"llmOutput": getattr(v, "llm_output", "") or ""},
        ),
        OpSpec(
            id="list_guards",
            label="List Guards",
