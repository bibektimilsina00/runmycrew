"""AWS CodePipeline action node — AWS CodePipeline — start / stop / inspect pipelines.

REST at https://codepipeline.{region}.amazonaws.com. See sim-parity roadmap Phase 4.30.
"""
...
MANIFEST = ProviderManifest(
    type="action.codepipeline",
    name="AWS CodePipeline",
    category="integration",
    description="AWS CodePipeline — start / stop / inspect pipelines.",
    icon_slug="codepipeline",
    color="#ffffff",
    base_url="https://codepipeline.{region}.amazonaws.com",
    ...
    operations=[
        ...
        OpSpec(
            ...
            label="Get Pipeline State",
            method="POST",
            path="/",
            visible_fields=["pipeline_name"],
            body_builder=lambda v: {"name": getattr(v, "pipeline_name", "") or ""},
        ),
        OpSpec(
            id="start_pipeline_execution",
            label="Start Pipeline Execution",
            method="POST",
            path="/",
            visible_fields=["pipeline_name"],
            body_builder=lambda v: {"name": getattr(v, "pipeline_name", "") or ""},
