"""Mistral OCR action node — Mistral OCR — document parsing to markdown / structured data.

REST at https://api.mistral.ai/v1. See sim-parity roadmap Phase 4.18/4.19.
"""
...
MANIFEST = ProviderManifest(
    ...
    operations=[
        OpSpec(
            ...
            method="POST",
            path="/ocr",
            visible_fields=["url", "model"],
            body_builder=lambda v: {
                "model": getattr(v, "model", None) or "mistral-ocr-latest",
                "document": {"type": "document_url", "document_url": getattr(v, "url", "") or ""},
            },
        ),
        OpSpec(
            id="upload_file",
            label="Upload File for OCR",
            method="POST",
            path="/files",
            visible_fields=["file_id"],
            body_builder=lambda v: {"purpose": "ocr"},
        ),
    ],
    outputs_schema=[
