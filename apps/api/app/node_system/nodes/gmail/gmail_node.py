            ...
            async with httpx.AsyncClient(timeout=30) as client:
                if op == "send_email":
                    if not self.props.to:
                        return NodeResult(success=False, error="To address required")
                    msg = MIMEMultipart()
                    msg["to"] = self.props.to
                    msg["subject"] = self.props.subject or ""
                    if self.props.cc:
                        msg["cc"] = self.props.cc
                    if self.props.bcc:
                        msg["bcc"] = self.props.bcc
                    msg.attach(MIMEText(self.props.body or "", self.props.body_type))
                    raw = base64.urlsafe_b64encode(msg.as_bytes()).decode()
                    r = await client.post(
                        f"{GMAIL_API}/users/me/messages/send", headers=headers, json={"raw": raw}
                    )
                    r.raise_for_status()
                    ...
                    params = {
                        ...
                        "maxResults": min(self.props.max_results, 500),
                    }
                    r = await client.get(
                    ...
                    if not self.props.message_id:
                        return NodeResult(success=False, error="message_id required")
                    r = await client.get(
                    ...
                    r.raise_for_status()
                    return NodeResult(
                        success=True, output_data={"labels": r.json().get("labels", [])}
                    )
                elif op == "get_profile":
