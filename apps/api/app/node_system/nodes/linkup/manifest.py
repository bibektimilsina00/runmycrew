"""Linkup action node — Linkup — real-time web search (better recall than SerpAPI).

REST at https://api.linkup.so/v1. See sim-parity roadmap Phase 4.32.
"""
...
MANIFEST = ProviderManifest(
    ...
    credential_type="linkup_api_key",
    token_field=["api_key"],
    auth="bearer",
    fields=[
    ...
    operations=[
        OpSpec(
            ...
            method="POST",
            path="/search",
            visible_fields=["query", "depth", "output_type"],
            body_builder=lambda v: {
                "q": getattr(v, "query", "") or "",
                "depth": getattr(v, "depth", None) or "standard",
                "outputType": getattr(v, "output_type", None) or "searchResults",
            },
        ),
    ],
    outputs_schema=[
