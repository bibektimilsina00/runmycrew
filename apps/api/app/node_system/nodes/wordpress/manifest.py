...
MANIFEST = ProviderManifest(
    ...
    fields=[
        ...
        FieldSpec(name="media_id", label="Media ID", type="string"),
        FieldSpec(name="media_source_url", label="Media Source URL", type="string"),
        FieldSpec(name="comment_id", label="Comment ID", type="string"),
        FieldSpec(name="comment_content", label="Comment Content", type="string"),
        FieldSpec(name="author_name", label="Author Name", type="string"),
        FieldSpec(name="author_email", label="Author Email", type="string"),
        FieldSpec(name="category_name", label="Category Name", type="string"),
    ...
    operations=[
        ...
        OpSpec(
            ...
            method="POST",
            path="/pages/{page_id}",
            visible_fields=["page_id", "title", "content", "status"],
            body_builder=lambda v: {
                k: val
                for k, val in {
                    "title": getattr(v, "title", None) or None,
        ...
        OpSpec(
            id="upload_media",
            label="Upload Media (URL fetch)",
            method="POST",
            path="/media",
            visible_fields=["media_source_url", "title"],
            body_builder=lambda v: {
                "source_url": getattr(v, "media_source_url", "") or "",
                "title": getattr(v, "title", None) or None,
            },
        ),
        OpSpec(
        ...
        OpSpec(
            ...
            query_builder=lambda v: {"per_page": int(getattr(v, "per_page", 10) or 10)},
        ),
        OpSpec(
            id="delete_media",
            label="Delete Media",
            ...
            path="/media/{media_id}",
            visible_fields=["media_id"],
            query_builder=lambda v: {"force": True},
        ),
        OpSpec(
            id="create_comment",
            ...
            method="POST",
            path="/comments",
            visible_fields=["post_id", "comment_content", "author_name", "author_email"],
            body_builder=lambda v: {
                "post": int(getattr(v, "post_id", 0) or 0),
                "content": getattr(v, "comment_content", "") or "",
                "author_name": getattr(v, "author_name", None) or None,
                "author_email": getattr(v, "author_email", None) or None,
            },
        ),
        OpSpec(
            id="list_comments",
            ...
            query_builder=lambda v: {
                k: val
                for k, val in {"post": int(getattr(v, "post_id", 0) or 0) or None}.items()
                if val
            },
        ),
        OpSpec(
            ...
            visible_fields=["comment_id", "comment_content"],
            body_builder=lambda v: {"content": getattr(v, "comment_content", "") or ""},
        ),
        OpSpec(
            id="delete_comment",
        ...
        OpSpec(
            ...
            method="POST",
            path="/categories",
            visible_fields=["category_name"],
            body_builder=lambda v: {"name": getattr(v, "category_name", "") or ""},
        ),
        OpSpec(
            id="list_categories",
        ...
        OpSpec(
            ...
            query_builder=lambda v: {"search": getattr(v, "query", "") or ""},
        ),
    ],
    outputs_schema=[
