...
from apps.api.app.features.collaboration.schemas import (
...
class CollaborationService:
    ...
    async def authenticate(
        ...
        workflow = await WorkflowRepository(self.db).get_by_id_and_workspace(
    ...
    async def add_presence(self, workflow_id: uuid.UUID, session: CollaborationSession) -> None:
        ...
        await redis.hset(
            await self.presence_key(workflow_id), session.session_id, session.model_dump_json()
        )
        await redis.expire(
            await self.presence_key(workflow_id), 300
        )  # 5-min safety net — primary cleanup via timeout
    ...
    async def run_socket(
        ...
        listen_task = asyncio.create_task(
            self.forward_pubsub(websocket, workflow_id, session.session_id)
        )
        receive_task = asyncio.create_task(
            self.receive_and_publish(websocket, workflow_id, session)
        )
        try:
