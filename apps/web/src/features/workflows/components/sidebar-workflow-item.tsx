...
export function SidebarWorkflowItem({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex items-center gap-[9px] pt-[6px] pr-[6px] pb-[6px] pl-[12px] rounded-[8px] text-[12.5px] text-[var(--text-mute)] cursor-pointer transition-colors duration-100 w-full text-left font-medium hover:bg-[var(--surface)] hover:text-[var(--text)] group/wf",
          isWfActive && "bg-[var(--surface)] text-[var(--text)] before:content-[''] before:w-[3px] before:h-[14px] before:bg-[var(--text)] before:rounded-[0_2px_2px_0] before:absolute before:left-0",
      ...
      >
        <span
          ...
          style={{
            backgroundColor: getWorkflowColor(workflow),
            boxShadow: `0 0 6px ${getWorkflowColor(workflow)}80`,
          }}
        />
          ...
          >
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                closeMenu()
                navigate(APP_ROUTES.WORKFLOW(workflow.id))
              }}
            >
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                closeMenu()
                navigate(`/runs?workflowId=${workflow.id}`)
              }}
            >
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                closeMenu()
                onRename(workflow.id, workflow.name, workflow.color || null)
              }}
            >
              <Icons.Edit /> Rename
            </button>
